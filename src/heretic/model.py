        ...
        for layer_index in range(len(self.get_layers())):
            for component, modules in self.get_layer_modules(layer_index).items():
                ...
                for module in modules:
                    ...
                    if self.settings.use_ridge_probe and probes is not None:
                        # Dynamic Ridge Steering Gate Logic
                        w_probe = probes[f"{layer_index}.{component}"].to(module.weight.device)
                        lora_A = w_probe.view(1, -1)
                        # We negate weight so the refusal direction is subtracted when the gate is active
                        lora_B = (-weight * v).view(-1, 1)

                        if self.settings.row_normalization == RowNormalization.PRE:
                            # Pre-normalization with ridge probes is technically tricky but we keep the row norms.
                    ...
                    if self.settings.row_normalization == RowNormalization.FULL:
                        # Only original projection steering supports full row norm approximation via SVD currently
                        if self.settings.use_ridge_probe:
                            pass # Skip full normalization for ridge probe
                        else:
                            pass # will be processed by next block
                    if not self.settings.use_ridge_probe and self.settings.row_normalization == RowNormalization.FULL:
